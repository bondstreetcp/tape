...
async function main() {
    ...
    moverRows.map(async (s) => {
      ...
      if (rep) {
        ...
        driver =
          ...
          (preEarnings
            ? ` ⚠ AMC — the shown ${pct(ret1d)} close-to-close move PRE-DATES the earnings (NOT the reaction): attribute it to the tape/sector/positioning (MOVE EVIDENCE below), and treat the print as a separate after-hours/next-session event`
            : "") +
          (driver ? ` · ${driver}` : "");
      }
  ...
  const filings = (overnight?.items ?? [])
    ...
    .map((f) => {
      ...
      const t = f.reportTiming === "afterhours" ? " · AMC" : f.reportTiming === "premarket" ? " · BMO" : "";
