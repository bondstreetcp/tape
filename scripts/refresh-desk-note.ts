...
const SCHEMA_HINT =
  'Return ONLY JSON: {"tldr": string (2-3 sentences: the tape + the single most important thing), "sections": [{"heading": string, "synthesis": string (the thematic read), "bullets": [{"fact": string (ONE company/event, a short scannable one-line title), "read": string (the SECOND LAYER — why it matters / read-through / signal-vs-noise / what to watch), "tickers": string[], "tag": string}]}], "watchToday": [{"text": string, "tickers": string[]}]}';
