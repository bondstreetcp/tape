...
const CALLS_ARCHIVE = path.join(ROOT, "lib", "callsArchive.ts"); // reads data/calls/** (the definer)
const CALLS_ARCHIVE_SYNC = path.join(ROOT, "lib", "callsArchiveSync.ts"); // script-only R2 tarball sync — never in a route graph
...
// data/calls/** — the earnings-call archive (lib/callsArchive.ts). Consumers: the transcripts page (loadSymbolCalls),
// lib/ask.ts + lib/earningsPreview.ts (getCallDigestHistory). Over-tainting is SAFE (a route just keeps the include);
// under-tainting is not — so the reliable signal is the distinctive read-fn names, with the path literal as a backstop.
const usesCalls = (f) => (/loadSymbolCalls|getCallDigestHistory|loadCallRecord|hasCallRecord|countCallRecords/.test(read(f)) && f !== CALLS_ARCHIVE) || (/["']calls["']\s*\)|data[\\/ ]+calls|["']calls["']\s*,/.test(read(f)) && f !== CALLS_ARCHIVE_SYNC);
function analyze(route) {
  const seen = new Set([route]), stack = [route]; let series = false, company = false, calls = false;
  while (stack.length) { const f = stack.pop(); if (usesSeries(f)) series = true; if (usesCompany(f)) company = true; if (usesCalls(f)) calls = true; for (const i of importsOf(f)) if (!seen.has(i)) { seen.add(i); stack.push(i); } }
  return { series, company, calls };
}
...
const callsIds = routes.filter((r) => r.calls).map((r) => r.id);
...
const excludes = {};
let slimSeries = 0, slimCompany = 0, slimCalls = 0, unsafe = [];
for (const r of routes) {
  ...
  if (!r.calls) { if (keyIsSafe(r.key, callsIds)) { dirs.push(CALLS); slimCalls++; } else unsafe.push([r.key, "calls"]); }
...
for (const [key, dirs] of Object.entries(excludes)) {
  ...
  if (dirs.includes(CALLS)) for (const id of callsIds) if (isM(id)) { console.error("VIOLATION: calls-excluded key", key, "matches calls route", id); violations++; }
}
// prove the dir globs match the right FILES and not base files (resolved like Next does: join(dir, glob))
const fileMatch = picomatch([path.join(ROOT, "./data/series/**"), path.join(ROOT, "./data/company/**"), path.join(ROOT, "./data/calls/**")].map((p) => p.replace(/\\/g, "/")), { dot: true, contains: true });
const seriesFile = (ROOT + "/data/series/symbols/AAPL.json").replace(/\\/g, "/");
const companyFile = (ROOT + "/data/company/AAPL.json").replace(/\\/g, "/");
const callsFile = (ROOT + "/data/calls/AAPL/2026-Q2.json").replace(/\\/g, "/");
const baseFile1 = (ROOT + "/data/market.json").replace(/\\/g, "/");
const baseFile2 = (ROOT + "/data/sp500/snapshot.json").replace(/\\/g, "/");
console.log("\n=== SUMMARY ===");
console.log("routes:", routes.length, "| series-needing:", seriesIds.length, "| company-needing:", companyIds.length, "| calls-needing:", callsIds.length);
console.log("excluded series from", slimSeries, "routes | excluded company from", slimCompany, "routes | excluded calls from", slimCalls, "routes");
...
console.log("calls file excluded?  ", fileMatch(callsFile), "(want true)");
