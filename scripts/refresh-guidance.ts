...
const SYSTEM =
  ...
  "'epsLow'/'epsHigh' = the EPS (earnings PER SHARE) guide range in DOLLARS — typically between $0.05 and $50. Do NOT return a margin %, a revenue/sales figure, a growth rate, or any non-per-share number as EPS. '$6.20–$6.40' → 6.20 / 6.40; adjusted/non-GAAP is fine, note it in metricLabel. Store the company's stated guidance RANGE (low & high); if it guides 'to the low/high end of $X to $Y' or 'approximately $X (within a $X to $Y range)', store the full $X–$Y range, not the single point. null if no EPS guide. " +
  "'action' = how this outlook compares to the company's PRIOR guidance, based STRICTLY on the filing's OWN words — do NOT infer a direction by eyeballing whether a number looks higher: 'raise' (says raised / increased / now expects higher / above prior), 'cut' (lowered / reduced / narrowed to the low end / below prior), 'reaffirm' (reiterated / maintained / reaffirmed / unchanged / on track), 'initiate' (first-time or newly-introduced guidance), 'mixed' (raised one metric while cutting another). Use 'none' when the filing states NO comparison to prior guidance — never guess a direction. " +
