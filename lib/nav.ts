...
export type NavGroup = "Markets" | "Earnings" | "Options" | "Event-Driven" | "Screens" | "Research";
...
export const FEATURES: NavItem[] = [
  ...
  { label: "Market Wire", path: "/wire", desc: "Live macro/market/geopolitical headlines — Walter Bloomberg's curated flashes + Reuters/Bloomberg/CNBC, refreshed ~every 2 min", group: "Markets", job: "Track the market", kw: "news wire headlines walter bloomberg deltaone reuters bloomberg cnbc wsj macro fed trade tariffs opec geopolitics breaking flash live ticker crossing" },
  ...
  { label: "Earnings Calendar", path: "/earnings", desc: "Who reports when, with the options-implied move", group: "Earnings", job: "Find ideas", kw: "earnings dates report calendar" },
  ...
  { label: "Sell-Premium Board", path: "/sell-premium", desc: "The best names to sell option premium on right now — one composite score across variance premium, IV rank & peer-relative richness, haircut for earnings traps", group: "Options", job: "Income strategies", kw: "sell premium board leaderboard where to sell theta wheel cash secured put covered call variance premium iv rank rich vol sector relative composite score ranked best names income earnings trap clear overwrite decision" },
  ...
  { label: "Earnings Setup Cards", path: "/earnings-setup", desc: "Glanceable cards of upcoming reporters — implied vs. historical move, rich/cheap", group: "Earnings", job: "Find ideas", kw: "earnings setup cards implied move straddle reporting this week" },
  { label: "Earnings Play Track Record", path: "/track-record", desc: "How the earnings card's suggested option plays have actually done — logged live, settled at expiry", group: "Earnings", job: "Find ideas", kw: "track record performance earnings play trade log scorecard win rate pnl settled straddle strangle results hit rate accountability" },
  { label: "Preview Accuracy Record", path: "/preview-record", desc: "The desk's own predicted prints — EPS, beat/miss and reaction calls logged before each report, graded against the actuals", group: "Earnings", job: "Find ideas", kw: "preview accuracy predicted print forecast eps beat miss reaction scorecard track record graded ai prediction earnings" },
  { label: "Earnings Print Predictor", path: "/print-predictor", desc: "A transparent model that reads each earnings call's tone & guidance to predict the NEXT print's beat/miss and price reaction — graded walk-forward, out-of-sample", group: "Earnings", job: "Find ideas", kw: "print predictor beat miss reaction earnings call transcript tone guidance forecast walk forward out of sample auc rank ic calibration logistic odds ratio next quarter model" },
  ...
  { label: "Guidance", path: "/guidance", desc: "Each company's standing guide + who sandbags (guides low, beats) vs over-promises its own outlook", group: "Earnings", job: "Find ideas", kw: "guidance outlook forecast raised reaffirmed cut lowered eps revenue range sandbagger sandbag beats own guide track record forward guide management credibility conservative aggressive earnings" },
  { label: "Earnings Season Desk", path: "/earnings-desk", desc: "The one-page hub — this week's prints, where options are mispriced into them, who's drifting, and the standing setups, all collated", group: "Earnings", job: "Find ideas", kw: "earnings season desk hub dashboard overview home landing collate everything one place calendar this week options mispriced drift sandbaggers vol summary digest command center" },
  ...
  { label: "Convertible Watch", path: "/convertibles", desc: "Recent convertible-note issuance (the AI-capex trade) — implied issue vol vs listed IV (cheap/rich) + the conversion-price & capped-call dilution levels", group: "Options", job: "Find ideas", kw: "convertible bond note issuance convert arb capped call call spread bond hedge conversion price premium coupon issue vol cheap rich dilution ai capex data center financing embedded option parity delta gamma mstr strategy" },
  { label: "Earnings This Week", path: "/earnings-week", desc: "The week-ahead earnings calendar with each name's implied expected move + rich/cheap", group: "Earnings", job: "Find ideas", kw: "earnings calendar this week upcoming reporters expected move straddle implied rich cheap bmo amc before open after close schedule when reporting" },
  ...
  { label: "Post-Earnings Drift", path: "/pead", desc: "Names still drifting after the print — the earnings-day gap vs the drift since", group: "Earnings", job: "Find ideas", kw: "pead post earnings drift momentum gap reaction continuation surprise fade reported recently after the print sue" },
  ...
  { label: "Staples Scanner", path: "/staples-scanner", desc: "NielsenIQ US retail demand & share for consumer staples — $ growth, volume/price, and share momentum across 2/4/12/52-week windows; a leading read on the quarter, ahead of the print", group: "Screens", job: "Find ideas", kw: "nielsen niq scanner retail staples consumer beverages beer spirits wine tobacco cigarettes nicotine pouches beauty CSD energy soda snacks share momentum inflection demand tracker point of sale scanner data KO PEP KDP MO PM STZ TAP MNST CELH" },
  ...
  { label: "Short Mechanics", path: "/short-mechanics", desc: "Daily short-volume % (FINRA) + fails-to-deliver (SEC) — the short-side plumbing behind the squeeze", group: "Screens", job: "Find ideas", kw: "short volume finra reg sho fails to deliver ftd settlement hard to borrow off exchange short selling mechanics daily activity" },
  ...
  { label: "Comp Stack Analyzer", path: "/comp-stacks", desc: "What each retailer's comp guide implies for the 2-year stack over the rest of its fiscal year — guided and back-solved quarters vs simply holding the stack", group: "Screens", job: "Find ideas", kw: "two year stack 2-yr stack comp guide implied comps rest of year fiscal year sandbag conservative guidance retail restaurants same store sales bogey hold the stack deceleration" },
  ...
  { label: "Market News Tape", path: "/news", desc: "Company press releases and SEC filings as they hit the public wires — ticker-tagged, searchable, and archived so you can scroll back through what already happened", group: "Research", job: "Track the market", kw: "news tape live news real time market news wire headlines breaking press release pr newswire globenewswire globe newswire sec edgar 8-k filings feed stream ticker tagged headline scanner squawk news flow archive rewind what happened newsfeed" },
  { label: "Key Debates", path: "/debates", desc: "The big arguments — AI capex durability, the memory cycle, GLP-1 spillover, rate cuts — each as a dated ledger of evidence for and against, drawn from SEC filings and short theses", group: "Research", job: "Research a name", kw: "debate thesis bull bear argument evidence ledger key debate ai capex memory supercycle glp-1 rate cuts both sides pro con for against roster thesis tracker" },
  ...
  { label: "Free Options", path: "/free-options", desc: "Reasonably-priced stocks with under-appreciated multi-year earnings power — growth handed to you cheap (low PEG / 3-yr-out P/E), with rising estimates and a real revenue runway", group: "Screens", job: "Find ideas", kw: "free option free options value growth garp peg cheap under-appreciated earnings power revenue runway 3 year multi-year compounder mispriced reasonable multiple forward growth priced reasonably" },
  // Discount-to-NAV + relative-value scanners — moved out of Research's "Portfolio & Tools" hub
  // (2026-07-24, Sam): they're market-wide screens, not your-book tools.
  { label: "CEF Screener", path: "/cef", desc: "Closed-end funds trading at a discount to their NAV", group: "Screens", job: "Income strategies", kw: "closed end funds discount nav yield" },
  { label: "CEF Discount Hunter", path: "/cef-hunter", desc: "The scored shortlist of the most stretched closed-end-fund discounts", group: "Screens", job: "Income strategies", kw: "cef closed end fund discount hunter stretched z-score yield" },
  { label: "Holdco NAV Tracker", path: "/holdco-nav", desc: "Holding companies vs their look-through NAV — discount/premium (Prosus, Exor, GBL…)", group: "Screens", job: "Find ideas", kw: "holdco holding company nav discount premium look-through prosus exor gbl arbitrage sum of the parts sotp" },
  { label: "Pairs (Relative Value)", path: "/pairs", desc: "Same-sector S&P 500 pairs whose spread is stretched and mean-reverts — the classic stat-arb setup, ranked by z-score", group: "Screens", job: "Find ideas", kw: "pairs trading relative value stat arb statistical arbitrage cointegration spread z-score mean reversion hedge ratio long short market neutral convergence divergence half life correlation pair trade" },
  ...
  { label: "Lobbying", path: "/lobbying", desc: "Who spends on Washington, on which bills, and whether Congress trades the same names — LDA filings joined to bill status and STOCK Act trades", group: "Research", job: "Find ideas", kw: "lobbying lobbyist lda disclosure bills legislation congress washington policy influence spend registrant client contested" },
  ...
  { label: "Merger Arbitrage", path: "/merger-arb", external: "https://arb.truporchhomesvm.com", desc: "Our dedicated merger-arb tool — live cash & stock deal spreads with the full path-to-close (opens the ARB site)", group: "Event-Driven", job: "Find ideas", kw: "merger arbitrage arb deal spread definitive agreement defm14a acquisition takeover cash deal annualized return break risk pending close special situations" },
  ...
  { label: "Value Chains", path: "/chains", desc: "Supply chains as ordered layers — who sells to whom, with margins, concentration and coverage per layer", group: "Screens", job: "Find ideas", kw: "value chain supply chain layers upstream downstream ai datacenter housing aerospace oil gas grid power drug pharma margins hhi concentration who benefits" },
  ...
  { label: "Conferences", path: "/conferences", desc: "Paste a conference link to capture presentations, transcripts and shareholder summaries", group: "Research", job: "Research a name", kw: "conference webcast audio whisper transcript fireside presentation upload" },
...
export const GROUP_HUBS: Partial<Record<NavGroup, NavHub[]>> = {
  Markets: [
    ...
    { label: "Movers & Buzz", blurb: "Relative-strength leaders, unusual options flow, retail buzz", paths: ["/leaders", "/flow", "/reddit-buzz"] },
  ],
  Earnings: [
    { label: "Season Desk", blurb: "The one-page earnings hub — this week's prints, where options are mispriced into them, who's drifting, and the standing setups, all collated", paths: ["/earnings-desk"] },
    { label: "Calendar & setups", blurb: "Who reports when + glanceable setup cards — the options-implied vs. historical move, rich or cheap into the print", paths: ["/earnings", "/earnings-week", "/earnings-setup"] },
    { label: "Trade the print", blurb: "Where options over/under-price the event, the prediction-market P(beat), post-earnings drift, and who sandbags their own guide", paths: ["/earnings-move", "/earnings-odds", "/pead", "/guidance"] },
    { label: "Track records", blurb: "Accountability — how the suggested option plays have actually done, and how the desk's own predicted prints graded vs the actuals", paths: ["/track-record", "/preview-record"] },
  ],
  Options: [
    { label: "Desks", blurb: "The weekly AI Trade Desk, the forward Catalyst Calendar, and this week's ranked binary events", paths: ["/trade-desk", "/catalyst-calendar", "/binary-week"] },
    ...
    { label: "Vol & Positioning", blurb: "The realized-vol cone, where vol is rich/cheap, skew & term structure, dispersion, dealer gamma, the name-level positioning radar, coiled springs, cheap options into a catalyst (incl. biotech binaries), and convertible issuance", paths: ["/vol-cone", "/vol-dislocation", "/skew", "/term-structure", "/dispersion", "/gamma-board", "/positioning", "/coiled", "/catalyst-vol", "/biotech-vol", "/convertibles"] },
    { label: "Income", blurb: "Sell premium on quality names — the ranked sell-premium board, cash-secured puts, covered calls, credit spreads, and a tracker for your active wheels", paths: ["/sell-premium", "/put-writing", "/covered-call", "/credit-spreads", "/wheel"] },
  ],
  ...
  "Event-Driven": [
    ...
    { label: "Merger Arbitrage", blurb: "Our dedicated ARB tool for deal spreads (opens externally) + live tender offers with the odd-lot lens + SPACs below trust value", paths: ["/merger-arb", "/tenders", "/spac-arb"] },
  ...
  Screens: [
    ...
    { label: "Staples Scanner", blurb: "NielsenIQ US retail demand & share for staples — $ growth, volume/price, and share momentum by 2/4/12/52-wk window; a leading read on the quarter", paths: ["/staples-scanner"] },
    { label: "Idea Scanners", blurb: "The Idea Inbox — fresh arrivals across every board, weighted by each board's graded record — plus the signal-fusion boards themselves and the live track record grading them", paths: ["/ideas", "/confluence", "/warnings", "/signal-record", "/smart-money", "/revisions", "/analyst-upside", "/squeeze", "/short-mechanics", "/gov-contracts", "/chains", "/insiders", "/factor-overlap", "/comps", "/comp-stacks", "/pairs"] },
  ...
  Research: [
    { label: "Conferences", blurb: "Capture conference presentations and publish transcripts with shareholder summaries", paths: ["/conferences"] },
    ...
    { label: "A Theme", blurb: "The market's live arguments, each as a dated ledger of evidence for and against", paths: ["/debates"] },
    { label: "What's Happening Now", blurb: "The live wire tape — press releases and SEC filings as they land, tagged to tickers and archived so you can scroll back", paths: ["/news"] },
    { label: "Ownership", blurb: "Who owns it + who's trading it — super-investor 13F holdings + distribution, Congress trades, lobbying spend, Trump's stock calls", paths: ["/superinvestors", "/distribution", "/congress", "/lobbying", "/trump-stocks"] },
    // Pure Prism now — the market-wide discount/relative-value scanners (holdco, pairs, CEFs) moved
    // to Screens (2026-07-24, Sam: "that's not the right place for those").
    { label: "Prism (Your Book)", blurb: "Your names monitored — the change ledger since you last looked, the book's risk engine, factor attribution + solved hedges, the catalyst radar, and covered-call income on your longs", paths: ["/my-names", "/portfolio", "/portfolio-radar", "/portfolio-income"] },
  ],
};
...
export const US_ONLY_PATHS: ReadonlySet<string> = new Set([
  ...
  "/earnings-setup", "/track-record", "/preview-record", "/print-predictor", "/guidance", "/pead",
  "/vol-dislocation", "/skew", "/term-structure", "/dispersion", "/gamma-board", "/positioning", "/coiled", "/catalyst-vol", "/biotech-vol",
  "/sell-premium", // composite sell-premium board over vol-dislocation.json (US single-stock options)
  "/convertibles", // US convertible-note issuance from EDGAR + US listed-IV comparison
...
export const NAV_GROUPS: NavGroup[] = ["Markets", "Earnings", "Options", "Event-Driven", "Screens", "Research"];
