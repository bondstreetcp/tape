...
export async function backupPresentation(job: PortalJob, talkId: string) {
  ...
  await putObject(`site-data/conferences/${job.id}/manifest.json`,Buffer.from(JSON.stringify({id:job.id,title:job.title,records,updatedAt:new Date().toISOString()})),'application/json',15000);
  talk.backedUp=true;
}
