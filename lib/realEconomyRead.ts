...
export async function buildRealEconomyRead(data: RealEconomyData): Promise<RealEconomyRead | null> {
  ...
  const tsaLine = data.tsa
    ...
    : "";
  const sheet = [section("Activity"), section("Manufacturing"), section("Services"), section("Freight"), section("Consumer"), section("Labor"), section("Housing"), section("Travel"), tsaLine].filter(Boolean).join("\n\n");
