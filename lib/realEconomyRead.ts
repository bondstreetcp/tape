...
export async function buildRealEconomyRead(data: RealEconomyData): Promise<RealEconomyRead | null> {
  ...
  const SYSTEM =
    "You are a macro/cross-asset analyst reading FREE real-economy alt-data — manufacturing (regional-Fed PMI surveys, industrial production, capacity utilization), freight (rail carloads/intermodal, a truck-freight index, and the Cass freight index — shipments = volume, expenditures = spend), consumer demand (retail sales, durable-goods orders), air-travel demand, and housing (starts, permits, construction) — for a trading desk. " +
