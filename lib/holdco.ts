...
export const HOLDCOS: Holdco[] = [
  ...
  {
    slug: "liberty-global", name: "Liberty Global", ticker: "LBTYK", currency: "USD", sharesOutM: 338, netDebtM: 0, otherNavM: 8450, asOf: "2026-06",
    note: "Malone-family connectivity holdco — now almost ENTIRELY UNLISTED after a 2024-26 reshaping, so this is a STATIC analyst-SOTP mark, NOT a live look-through: coverage ~0% (no material listed stakes remain, so `otherNAV` carries the whole NAV and the discount moves only with the holdco price until this figure is refreshed). SOTP composition: ~50% of VMO2 (UK JV with Telefónica, unlisted — and not even on management's own valuation slide) + 100% of Ziggo Group (the ex-VodafoneZiggo NL + Telenet Benelux, bought in from Vodafone Feb-2026; spin-off/IPO targeted ~2027, which is the value-unlock catalyst) + Liberty Growth (~$3.4bn private venture portfolio, Dec-2025 — ITV, Lionsgate, Plume, Formula E et al.). Sunrise was spun off Nov-2024; the Vodafone Group stake was fully EXITED in 2025 and the ITV stake halved, so no listed stake is left to mark live. otherNavM = the NET analyst SOTP equity (~$25/sh, the CONSERVATIVE end of a $25-38 range — the honest bias is never to flatter the discount), already net of holdco/JV debt, so netDebt is folded in (0 here). ~338M shares (175.5M A + 13.0M B + 149.7M C, Apr-2026 10-Q). At LBTYK ~$12 that's a ~−50% discount — the classic wide Malone conglomerate discount. LOW confidence / SEED — verify against LG's own SOTP; coverage (and live tracking) will jump when Ziggo Group spins ~2027 and becomes a listed stake.",
    stakes: [],
  },
  {
