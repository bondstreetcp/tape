...
export const ENV_KNOBS: readonly EnvKnob[] = [
  ...
  K("BACKFILL_UNIVERSE", "tuning", "backfill-transcripts", "Universe to backfill: empty = the committed S&P 500 list; else a universe id (russell3000, russell1000, sp1500, nasdaq100) whose R2-hydrated snapshot supplies the names.", ""),
  K("BACKFILL_YEARS", "tuning", "backfill-transcripts", "Years of earnings-call history to pull from MarketBeat into the data/calls archive.", "3"),
  K("BACKFILL_DELAY_MS", "tuning", "backfill-transcripts", "Politeness delay (ms) between MarketBeat fetches.", "700"),
  K("BACKFILL_TICKER_LIMIT", "tuning", "backfill-transcripts", "Cap the names processed (0 = all) — for chunking/testing.", "0"),
  ...
  K("INGEST_PAUSE_PEAK", "tuning", "ingest-transcripts", "1 = pause the rig during Georgia Power on-peak (weekdays 14:00-19:00 ET, excl holidays, thru Sep 30) to dodge peak electricity pricing.", "0"),
  K("INGEST_ONLY", "tuning", "ingest-transcripts", "Comma/space-separated ticker subset to ingest; empty = all.", ""),
  K("INGEST_UNIVERSE", "tuning", "ingest-transcripts", "Restrict the ingest work queue to a universe's snapshot members (e.g. 'sp500'); intersects with INGEST_ONLY; empty = the whole calls archive.", ""),
  K("INGEST_DELAY_MS", "tuning", "ingest-transcripts", "Delay (ms) between digest calls (pace a one-sequence rig).", "0"),
  // Conference capture server (scripts/capture-server.mjs) — the one-click bookmarklet's local download listener.
  K("CAPTURE_PORT", "tuning", "capture-server", "Port the conference capture server listens on (loopback 127.0.0.1 only).", "8765"),
  K("AUDIO_DIR", "tuning", "capture-server", "Folder where the capture server saves downloaded talk audio (TICKER_DATE.mp3).", "~/communicopia-audio"),
  K("YTDLP", "tuning", "capture-server", "yt-dlp binary name/path the capture server invokes.", "yt-dlp"),
  K("REFERER", "tuning", "capture-server", "Referer header sent to the webcasts CDN (securehds returns HTTP 704 without it).", "https://event.webcasts.com/"),
  K("UA", "tuning", "capture-server", "User-agent the capture server sends to the CDN.", "Mozilla/5.0"),
  K("ALLOW_ORIGIN", "tuning", "capture-server", "Comma-separated origins allowed to POST to the capture server; '*' allows any (still loopback-bound).", "https://event.webcasts.com"),
  K("CAPTURE_TOKEN", "secret", "capture-server", "Optional shared secret the capture bookmarklet must send (X-Tape-Token); empty = no token required."),
