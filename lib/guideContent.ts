...
export const GUIDE_GROUPS: GuideGroup[] = [
  {
    ...
    "features": [
      ...
      },
      {
        ...
        "how": "The morning workflow in one page, the tabs in reading order — the last, Earnings Calls, is the newest: every earnings-call transcript from the last session, read in full on the desk's own local model and digested (tone, guidance posture, the numbers stated, the sharpest analyst exchanges, verbatim quotes), with a cross-call synthesis on top. (1) The Desk Brief: an AI model reads the night's already-collected data — the biggest stock moves and their catalysts, material SEC filings, unusually large options trades, and analyst rating changes — and writes a plain-language brief twice each weekday (a pre-open 'morning run' and a post-close 'evening run'). Each item is two layers: the fact (what happened) plus the read (why it might matter), and it's tagged by type. Move attribution is code-anchored: a name that just filed its results 8-K is marked as having REPORTED, so its move is explained by the print (an upgrade dated after the print is reaction, not cause), and a drift 2-7 days after a report reads as a continued post-earnings move rather than 'no specific catalyst'. The code picks the inputs; the model only summarizes and never gives a buy/sell call. (2) The News Wire: your WATCHLIST leads it — each watched name with its 1-day move, a REPORTED badge when a results 8-K is on record (so a post-print drift is never read as 'no catalyst'), any live deal/review flag, and its freshest dated headlines, ordered fresh-prints-first then by move size; quiet names collapse to one line. Below that, the day's Reuters newsletters (Morning News Call and The Day Ahead) parsed into readable market tables and an earnings schedule; the Reuters section is password-protected. (3) Overnight Filings: the AI desk notes on new material SEC filings vs each company's prior comparable — the same board as the standalone Overnight Filings page, in the morning path.",
        "metrics": [
          {
  ...
  },
  {
    ...
    "features": [
      ...
      },
      {
        ...
        "question": "Among companies being acquired, which offer the best annualized return for holding through the close — and how much of that return is really deal-break risk?",
        "how": "Merger-arb now lives in our dedicated ARB tool (opens externally at arb.truporchhomesvm.com) — the standalone site carries the full deal book, both cash and stock deals, with each deal's path-to-close (the conditions left, expected dates, and the break risk). The concept: when a company agrees to be bought it files a definitive merger proxy, and its stock settles just below the agreed price — the gap is the spread, the market paying you to carry the risk the deal falls through, and a wide spread signals doubt (regulators, financing, a rival bid). It's a classic small-account edge: the big clean cash deals get crowded by funds, the smaller and messier ones are left alone. (Tape still uses the deal data internally to hold back an earnings-vol play on any name under a signed deal.)",
        "metrics": [
          {
            "term": "Deal",
  ...
  },
  {
    ...
    "features": [
      ...
      },
      {
        ...
        "how": "The monitoring layer, in three tabs: Ledger (this), Quotes (the watchlist table), and Radar (every forward catalyst on your names, soonest first). The Ledger is the monitoring layer for your book + watchlist. The page joins every watched name against the app's existing feeds — no AI in this path, every line links to its source — and lists typed events, most-actionable first: a results 8-K on record (REPORTED), a preannouncement, a signed deal or live strategic review, an upcoming report with the options-priced move, an overnight filing note, insider cluster buying, short-mechanics stress (heavy fails-to-deliver or elevated short volume), borrow-fee tightening, notable 30-day estimate-revision activity, unusual options premium, and fresh dated headlines. A per-browser 'since you last looked' cursor stamps NEW on anything that appeared after your previous visit, so the page answers the monitoring question directly instead of making you re-scan boards. Names with no events collapse into one quiet line. The list is your watchlist plus your pasted Prism book, united — book names carry a LONG/SHORT chip from your net position, starred names a WATCH chip. Push alerts (optional, via the free ntfy app): exactly three event classes can notify your phone — a preannouncement, a new deal or activist 13D on one of your names, and a report within ~36 hours with the options-priced move — evaluated on the nightly run; everything else stays on this page by design.",
        "metrics": [
          {
            "term": "NEW",
