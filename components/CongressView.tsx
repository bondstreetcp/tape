...
export default function CongressView({ universe, data, known }: { universe: string; data: CongressData; known: string[] }) {
  ...
  return (
    <main className="mx-auto max-w-[84rem] px-4 py-6 sm:px-6">
      <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-[var(--text-3)]">
            U.S. Senate stock trades disclosed under the STOCK Act · {data.trades.length} transactions since {dt(data.since)} · amounts are disclosed brackets, filed up to ~45 days late · source <a href="https://efdsearch.senate.gov" target="_blank" rel="noreferrer" className="text-[#60a5fa] hover:underline">Senate eFD</a>
          </p>
        </div>
          ...
          <tbody>
            {rows.slice(0, 400).map((t, i) => (
              <tr key={i} className="border-b border-[var(--divider)] hover:bg-[var(--surface-hover)]">
                ...
                <td className="whitespace-nowrap px-4 py-1.5">{t.member}</td>
      ...
      <p className="mt-2 text-[11px] text-[var(--text-4)]">Self-reported under the STOCK Act and parsed from Senate e-filings; covers U.S.-ticker stock &amp; ETF transactions only (bonds, options and non-tickered assets excluded). House filings (mostly PDFs) aren&apos;t included yet. Not a signal of wrongdoing or a recommendation.</p>
    </main>
  );
}
