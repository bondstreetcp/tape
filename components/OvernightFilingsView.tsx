...
// `embedded` renders the board without its page chrome (<main>/back-link/h1/UniverseSwitcher) —
// the Daily Desk mounts it as its Overnight Filings tab, where the page already owns that frame.
export default function OvernightFilingsView({ universe, data, known, sectors = {}, related = {}, embedded = false }: { universe: string; data: OvernightData; known: string[]; sectors?: Record<string, string>; related?: Record<string, RelatedFiling[]>; embedded?: boolean }) {
  ...
  return (
    <Frame className={embedded ? "" : "mx-auto max-w-[72rem] px-4 py-6 sm:px-6"}>
      {embedded ? (
        <div className="mb-2 flex items-baseline justify-between gap-2 border-b border-[var(--divider)] pb-1.5">
          <h2 className="text-lg font-bold text-[var(--text)]">Overnight filings</h2>
          <span className="text-[11px] text-[var(--text-4)]">
            {universeItems.length} filing{universeItems.length !== 1 ? "s" : ""}{sinceLabel ? ` since ${sinceLabel}` : ""} · AI-summarized vs the prior comparable — spot-check against <a href="https://www.sec.gov/cgi-bin/browse-edgar?action=getcurrent" target="_blank" rel="noreferrer" className="text-[var(--accent)] hover:underline">EDGAR</a>
          </span>
        </div>
      ) : (
