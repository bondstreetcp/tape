...
import type { CurvePoint } from "@/lib/fred";
...
export default function FixedIncomeView({
  universe, curve, asOf, creditSeries,
}: {
  universe: string;
  curve: CurvePoint[];
  asOf: string;
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-6 sm:px-6">
      ...
      <CreditSpreads creditSeries={creditSeries} />

      <p className="mt-4 text-[11px] text-[var(--text-4)]">Source: Federal Reserve Economic Data (FRED). Refreshes with the macro snapshot (npm run refresh-macro).</p>
    </main>
  );
}
