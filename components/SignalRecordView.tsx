...
import {
  ...
  type HorizonKey, type SignalEvent, type SignalKey, type SignalSummary,
} from "@/lib/signalLog";
  ...
  return (
    <main className="mx-auto max-w-[84rem] px-4 py-6 sm:px-6">
      ...
      <HowToRead>
        ...
        <p><b>Honest limits:</b> boards are logged from the broadest US universe (Russell 3000 context), returns are price-only (no dividends), marks land on the first weekday on/after each horizon (never more than 2 weeks late — later marks are dropped, not mislabeled), and the record only starts accruing from {fmtDate(since)} — early numbers are small-sample noise, not a verdict. Decision-support, not advice.</p>
      </HowToRead>
