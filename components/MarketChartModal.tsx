...
export default function MarketChartModal({ tile, onClose, universe }: { tile: Tile; onClose: () => void; universe: string }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/ohlc/${encodeURIComponent(tile.sym)}?years=5`)
      .then((r) => r.json())
  ...
  const fmtVal = (v: number) =>
    tile.kind === "rate" ? `${v.toFixed(2)}%` : tile.kind === "fx" ? (v >= 50 ? v.toFixed(2) : v.toFixed(4)) : v.toLocaleString(undefined, { maximumFractionDigits: 2 });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={onClose}>
      <div className="w-full max-w-3xl rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-4 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="mb-3 flex items-start justify-between gap-3">
          <div>
            <div className="flex items-center gap-2">
              <h3 className="text-base font-bold text-[var(--text)]">{tile.name}</h3>
              <span className="font-mono text-[11px] text-[var(--text-4)]">{tile.sym}</span>
            </div>
            {last != null && (
        ...
        <p className="mt-1 text-[11px] text-[var(--text-4)]">Daily closes via Yahoo. Click outside or press Esc to close.</p>
      </div>
    </div>
  );
}

function LineChart({ series, up, fmtVal }: { series: Bar[]; up: boolean; fmtVal: (v: number) => string }) {
  if (series.length < 2) return <div className="py-16 text-center text-sm text-[var(--text-3)]">Not enough data for this range.</div>;
  const W = 760, H = 280, ML = 58, MR = 12, MT = 12, MB = 22;
