          ...
          <table className="w-full min-w-[880px] text-left text-[13px]">
            <thead className="border-b border-[var(--border)] text-[11px] uppercase tracking-wide text-[var(--text-4)]">
              <tr>
                ...
                <th className="px-2 py-2 text-right font-medium" title="The RemainCo leg: the PARENT's own total price return since it spun this SpinCo off (absolute, not vs the S&P) — the slimmed parent is the more liquid, investable half of the trade.">RemainCo since</th>
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.ticker} className="border-b border-[var(--border)] last:border-0 hover:bg-[var(--surface-2)]">
                  ...
                  <td className="px-2 py-2.5 text-right font-mono tabular-nums text-[12px]" title={r.remainCo?.parentReturnPct != null ? `Parent ${pct(r.remainCo.parentReturnPct)} absolute total return since ${fmtDate(r.spinDate)}` : "Parent series not priceable (e.g. a foreign/ADR parent)"}>
